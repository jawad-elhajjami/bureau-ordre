        ...
        @scope('cell_user_sent_to', $document)
            ...
            @if(auth()->user()->id == $document->recipient->id)
                ...
                @else 
                    <x-mary-popover>
                        <x-slot:trigger>
                            <div class="w-9 h-9 text-sm font-bold text-gray-600 flex items-center justify-center rounded-full border border-gray-400" style="background-color: {{ $document->recipient->color ?? 'rgb(168,85,247)' }};">{{ $document->recipient->initials }}</div>
                        </x-slot:trigger>
                        <x-slot:content>
